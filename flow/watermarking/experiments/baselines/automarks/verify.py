...
"""AutoMarks baseline -- verify.

Same parity-check protocol as ICMarks; per-cell axis (x or y) and target
parity bit are recorded in the embed CSV. Region containment of the
selected cells is logged as an auxiliary metric.

Run as:
  openroad -python -exit verify.py \\
      --odb /path/to/N_stage.odb \\
      --embed-csv /path/to/automarks_embed.csv \\
      --stage DRT
"""
...
def _log(msg: str) -> None:
    print(f"[{time.strftime('%H:%M:%S')} +{time.time()-_T0:6.1f}s] [automarks:verify] {msg}", flush=True)
...
def _parse_args(argv: List[str]) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="AutoMarks verifier")
...
def main(argv: List[str]) -> None:
    ...
    out_csv = args.out_csv or str(Path(args.odb).parent / f"automarks_verify_{args.stage}.csv")
    ...
    print(f"AUTOMARKS_VERIFY stage={args.stage}: K={K} accepted={accepted} Pc={pc:.4e} in_region={in_region}", flush=True)

...
# Kahng row-parity baseline (KahngMMP98/KahngLMM01) -- chained driver.
#
# Required env:
#   DESIGN          e.g. aes
...
#
# Optional env:
#   FLOW_VARIANT    output variant (default: baseline-kahng)
...
if [[ -z "${SINGULARITY_NAME:-}" ]]; then
  exec singularity exec -B /home -B /tmp -e "${SIF}" \
    ...
    FLOW_VARIANT="${FLOW_VARIANT:-baseline-kahng}" \
...
export FLOW_VARIANT="${FLOW_VARIANT:-baseline-kahng}"

WM_DIR="${FLOW_HOME}/watermarking"
GEN_KEY_DIR="${WM_DIR}/gen_key"

ts()  { date '+%Y-%m-%d %H:%M:%S'; }
log() { echo "[$(ts)] [kahng/run.sh] $*"; }
...
log "embedding Kahng row-parity watermark on ${PLATFORM}/${DESIGN}/${WM_FLOW_VARIANT} -> ${FLOW_VARIANT}"
...
OUT_ODB="${OUT_RES}/3_place_kahng.odb"
EMBED_CSV="${OUT_RES}/kahng_embed.csv"
...
if [[ -f "${DRT_ODB}" ]]; then
  ${OPENROAD_EXE} -python -exit "${SCRIPT_DIR}/verify.py" \
    ...
    --embed-csv "${EMBED_CSV}" \
    --stage DRT \
    --out-csv "${OUT_RES}/kahng_verify_DRT.csv"
else

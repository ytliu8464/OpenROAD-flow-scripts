...
Reads embed CSV (ground truth). Parity is ``seq_fanout(target_lcb) % 2``.
"""
...
class PairStageResult:
    observed_seq_fanout: Optional[int]
    observed_bit: Optional[int]
    repair_fanout_target_observed: Optional[int]
    repair_fanout_other_observed: Optional[int]
    tampered: bool
    satisfied: bool
    missing: bool
...
def _verify_one_stage(
    ...
    inst_map: Dict[str, object] = {
        inst.getName(): inst for inst in block.getInsts()
    }
    ...
    for wp in pairs:
        ...
        if inst is None:
            results.append(
                PairStageResult(
                    observed_seq_fanout=None,
                    observed_bit=None,
                    repair_fanout_target_observed=None,
                    repair_fanout_other_observed=None,
                    tampered=False,
                    satisfied=False,
                    missing=True,
                )
        ...
        try:
            ...
            failures.append(
                f"pair {wp.pair_idx}: seq_fanout failed for {wp.target_lcb} "
                f"in {label}: {e}"
            )
            continue
        ...
        results.append(
            PairStageResult(
                ...
                repair_fanout_other_observed=ro_obs,
                tampered=tampered,
                satisfied=ok,
                missing=False,
            )
        ...
        if not ok:
            failures.append(
                ...
                f"want={wp.target_bit} tampered={tampered} ({label})"
            )
        ...
        for i, wp in enumerate(pairs):
            ...
            for si in range(len(labels)):
                ...
                if r.missing:
                    ...
                    row.extend(
                        [
                            r.observed_seq_fanout
                            if r.observed_seq_fanout is not None
                            else "",
...
def main() -> int:
    ...
    for label, odb_path in stages:
        if not os.path.isfile(odb_path):
            print(
                f"{label:<18} ERROR: file not found: {odb_path}",
                file=sys.stderr,
            )
            any_fail = True
            continue
